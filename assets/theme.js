(function ($) {
  ...
  theme.Shopify = {
    convertCurrency: function (amount, baseCurrency) {
      const { active, rate } = window.Shopify.currency;
      if (baseCurrency === active) return amount;
    ...
    if (prefix == "destroy") {
      ...
      return $(this)
        ...
        .each(function () {
          ...
          //Create container
          var $optCont = $('<ul class="clickyboxes"/>')
            .attr("id", "clickyboxes-" + prefix)
            .data("select", $(this))
            .insertAfter(this);
  ...
  $.fn.selectReplace = function (leaveLabel) {
    return $(this)
      ...
      .each(function () {
        ...
        var initialText =
          $opts.filter(":selected").length > 0
            ? $opts.filter(":selected").text()
            : $opts.first().text();
        var $cont = $(this)
          .addClass("replaced")
          .wrap('<div class="pretty-select">')
          .parent()
          .addClass("id-" + $(this).attr("id"))
          .append(
        ...
        if ($(this).attr("id")) {
          ...
          if (
            ...
            ($selectTD.length == 0 ||
              $labelTD.length == 0 ||
              $selectTD[0] == $labelTD[0])
          ) {
            //Add to dropdown
            var $labelSpan = $('<span class="label">')
              .html($label.html())
              .prependTo($cont.find(".text"));
        ...
        function updateLine() {
          let $link =
            arguments.length > 0 && arguments[0] !== undefined
              ? arguments[0]
              : $this.find('li a[aria-selected="true"], li a.active');
          if ($link.length === 1) {
            $hoverLine.css({
              width: $link.width(),
    ...
    theme.initAnimateOnScroll = function () {
      if (
        ...
        const animationTimeout =
          typeof document.body.dataset.ccAnimateTimeout !== "undefined"
            ...
            (entries, observer) => {
              entries.forEach((entry) => {
                ...
                if (
                  ...
                  !entry.target.classList.contains("cc-animate-complete")
                ) {
                  setTimeout(() => {
  ...
  class PriceRangeInstance {
    ...
    drawControl(control) {
      const x =
        (control.value - this.valueMin) * this.getPcToValueRatio() + "%";
      if (this.rtl) {
    ...
    incrementControlFromKeypress(control, pxAmount) {
      ...
      this.setActiveControlValue(
        this.activeControl.value + pxAmount / this.getPxToValueRatio()
      );
      this.render();
    }
          ...
          for (let mutation of mutations) {
            if (mutation.attributeName === "data-cc-sticky-scroll-top") {
              this.defaultTop = parseInt(
                mutation.target.dataset.ccStickyScrollTop
              );
            }
          }
        });
  ...
  class StickyScrollDirection extends ccComponent {
    ...
    init(container) {
      ...
      this.registerInstance(
        container,
        new StickyScrollDirectionInstance(container)
      );
    }
  ...
  new (class extends ccComponent {
    init(container) {
      ...
      $container.on("click", "[data-cc-toggle-panel]", function () {
        ...
        const $tabToOpen = $tabs.find(
          `.cc-tabs__tab .cc-tabs__tab__panel[aria-labelledby="product-tab-panel${tabIndexToShow}"]`
        );
        const openAllTabs = $(this).closest(
        ...
        if ($tabs.hasClass("cc-tabs--tab-mode")) {
          ...
          $tabs
            .find(`[data-cc-toggle-panel="${tabIndexToShow}"]`)
          ...
          if (!openAllTabs) {
            ...
            var tabNeedsOpening =
              $tabToOpen.attr("id") !== $tabToClose.attr("id") &&
              $tabToOpen.length;
          } else {
          ...
          if (tabNeedsOpening) {
            $tabToOpen
              .css("display", "none")
              .removeAttr("hidden")
              .slideDown(accordionSpeed, dispatchTabChangedEvent);
  ...
  theme.addDelegateEventListener = function (
    element,
    eventName,
    selector,
    callback
  ) {
    let addEventListenerParams =
  ...
  theme.mergeNodes = (newContent, targetContainer) => {
    try {
      ...
      newContent
        .querySelectorAll("[data-merge-attributes]")
        .forEach((newEl) => {
          const targetEl = targetContainer.querySelector(
      ...
      newContent.querySelectorAll("[data-merge-list]").forEach((newList) => {
        ...
        targetListItems.forEach((targetListItem) => {
          let matchedItem = newListItems.find(
            (item) =>
        ...
        for (let i = 0; i < newListItems.length; i++) {
          ...
          let matchedItem = targetListItems.find(
            (item) =>
    ...
    scroll: {
      ...
      to: function ($elem) {
        ...
        if (scrollTop === -1 && $elem && $elem.length) {
          const navHeight = theme.Nav().bar.isInline()
            ? 0
    ...
    adjustItemQuantity(item, change) {
      ...
      this.adjustItemQuantityTimeout = setTimeout(
        () => {
          ...
          (_this$querySelector = this.querySelector(".cart-list")) === null ||
            _this$querySelector === void 0 ||
            _this$querySelector.classList.add("cart-list--loading");
          ...
          fetch(`${window.Shopify.routes.root}cart/change.js`, {
            ...
            .then((response) => {
              ...
              $.get(theme.routes.cart_url, function (data) {
                ...
                var $newCartObj = $(
                  $.parseHTML("<div>" + data + "</div>")
                ).find(cartUpdateSelector);
                  ...
                  try {
                    ...
                    const slideDownAndFadeIn = () => {
                      ...
                      errorContainer.style.height =
                        errorContainer.scrollHeight + "px";
            ...
            .finally(() => {
              ...
              (_this$querySelector2 = this.querySelector(".cart-list")) ===
                null ||
                _this$querySelector2 === void 0 ||
                _this$querySelector2.classList.remove("cart-list--loading");
            });
        },
  ...
  theme.Nav = function () {
    ...
    return {
      bar: {
        ...
        isAnnouncementBar: () => {
          return (
            $("#site-control").find(".cc-announcement__inner").length === 1
          );
        },
        hasLocalization: () => {
        ...
        heightExcludingAnnouncementBar: () => {
          return Math.round(
            $("#site-control").find(".site-control__inner").outerHeight()
          );
        },
        heightOfAnnouncementBar: () => {
          return Math.round(
            $("#site-control").find(".announcement").outerHeight()
          );
        },
        height: () => {
          ...
          let height;
          if (
            $nav.hasClass("announcement-hidden") &&
            $nav.find(".cc-announcement__inner").length
          ) {
            height = Math.round(
              $nav.find(".cc-announcement__inner").outerHeight() +
                $nav.find(".site-control__inner").outerHeight()
            );
          } else {
  ...
  theme.ProductMediaGallery = function (
    $gallery,
    $thumbs,
    isFeaturedProduct,
    isQuickbuy,
    ...
    var isGalleryNarrow = $gallery
      .closest(".product-area")
      .hasClass("product-area--restrict-width");
    var $productThumbnails = $gallery
      .closest(".product-area")
      .find(".product-area__thumbs");
          ...
          if (vimeoApiReady) {
            if ($elem.data("enable-video-looping")) {
              iframeElement.setAttribute(
                "src",
                iframeElement.getAttribute("src") + "&loop=1"
              );
            }
            ...
            if (autoplay && $(window).width() >= 768) {
              iframeElement.setAttribute(
                "src",
                iframeElement.getAttribute("src") + "&autoplay=1&muted=1"
              );
            }
    ...
    this.destroy = function () {
      ...
      if (!isCarouselLayout) {
        $(window).off(
          `load.productTemplateGallery${galleryId} scroll.productTemplateGallery${galleryId}`,
      ...
      $gallery
        .closest(".product-area")
        .off("click", ".product-area__thumbs__thumb a", handleThumbnailClick);
    ...
    this.slideshowTabFix = function () {
      if (swiper) {
        ...
        $activeSlide
          ...
          .each(function () {
            if (
              typeof $(this).data("theme-slideshow-original-tabindex") !==
              "undefined"
            ) {
    ...
    this.scrollToMedia = function (mediaId) {
      ...
      if (
        $variantImage.length &&
        ($("body").hasClass("template-product") || isQuickbuy) &&
        theme.viewport.isSm()
      ) {
        let offset = parseInt(
          $gallery.find(".theme-images").css("padding-top").replace("px", "")
        );
        let scrollAmount;
        ...
        if (!isQuickbuy) {
          ...
          if (
            (nav.bar.hasOpaqueSetting() && nav.bar.hasStickySetting()) ||
            (isGalleryNarrow && nav.bar.hasStickySetting()) ||
            ($gallery.data("column-count") > 1 &&
          ...
          if (
            ...
            nav.bar.getPositionSetting() === "peek" &&
            nav.bar.hasOpaqueSetting()
          ) {
            scrollAmount -= nav.bar.heightExcludingAnnouncementBar();
          }
          ...
          scrollAmount =
            $variantImage.offset().top -
            $(window).scrollTop() +
            $("#quick-buy-modal").scrollTop();
        }
    ...
    function detectHeaderOverGallery() {
      ...
      $("body").toggleClass(
        "header-over-gallery",
        $(window).scrollTop() < $gallery.height() - nav.bar.height()
      );
    }
    ...
    function initColumns() {
      ...
      let $elements = $gallery.find(
        ".theme-img:visible:not(.theme-img--collage-full)"
      );
        ...
        $elements.each(function (i) {
          if (offset === 0 || i < $elements.length - offset) {
            if (currentCol < Math.floor(i / elementsPerCol)) {
              $colWrapper = $(`<div class="media-column"></div>`).appendTo(
                $colContainer
              );
              currentCol++;
            }
    ...
    //Init swiper
    var $swiperExternalVideos = $swiperCont.find(
    ...
    function handleImageClick() {
      ...
      if (theme.viewport.isSm()) {
        let thisSmallSizeImageUrl = $(this).find(".rimage-wrapper > img")[0]
          .currentSrc;
        ...
        $allImages.each(function () {
          let smallSizeImageUrl = $(this).find(".rimage-wrapper > img")[0]
            .currentSrc;
          let fullSizeImageUrl = $(this).data("full-size");
          let extraAttrs =
            thisSmallSizeImageUrl === smallSizeImageUrl
              ? "id='zoom-image'"
              : "";
        ...
        showThemeModal(
          $(
           ...
           <div class='inner-scroller -out'>${imageHtml}</div>`),
          "product-image",
          function ($modal) {
            ...
            setTimeout(() => {
              ...
              $modal.scrollTop(
                $mainImage.position().top +
                  ($mainImage.outerHeight() / 2 - $modal.outerHeight() / 2)
              );
    ...
    if ($gallery.hasClass("theme-gallery--thumbs-enabled")) {
      $gallery
        .closest(".product-area")
        .on("click", ".product-area__thumbs__thumb a", handleThumbnailClick);
    }
    ...
    $(window)
      .off(`ccScrollToMedia.productTemplateGallery${galleryId}`)
      .on(
        `ccScrollToMedia.productTemplateGallery${galleryId}`,
        function (e, mediaId) {
          if (
            $gallery.data("scroll-to-variant-media") !== false ||
            theme.viewport.isXs()
          ) {
            setTimeout(() => {
              _this.scrollToMedia(mediaId);
    ...
    if (!isCarouselLayout) {
      ...
      $(window).on(
        `scroll.productTemplateGallery${galleryId}`,
        detectHeaderOverGallery
      );
    } else {
    ...
    function initThumbnails() {
      $(
        ".carousel-wrapper .carousel:not(.slick-initialized)",
        $productThumbnails
      ).each(function ($slick) {
        $(this)
          .on("init reInit setPosition", function () {
            ...
            if (lastSlide.length > 0) {
              var slideInnerWidth =
                lastSlide.position().left + lastSlide.outerWidth(true);
    ...
    function destroyThumbnails() {
      $(".carousel-wrapper .carousel", $productThumbnails).off(
    ...
    function toggleThumbnailVisibility() {
      $(".slick-slider", $productThumbnails).slick(
        "slickFilter",
        '[data-cc-hidden="false"]'
      );
    }
    // Hides the irrelevant variant media
    function initVariantImageGrouping() {
      const productData = theme.OptionManager.getProductData(
        null,
        $gallery.data("product-id")
      );
      ...
      if (
        ...
        const colorOptionIndex = getFirstMatchingOptionIndex(
          productData.options
        );
        ...
        if (colorOptionIndex > -1) {
          ...
          productData.variants.forEach((variant) => {
            if (variant.featured_media) {
              ...
              mediaByVariantColor[variant.featured_media.id].push(
          ...
          $gallery.on("variantImageSelected", (e, variant) => {
            ...
            if (previousColor != targetColor) {
              ...
              productData.media.forEach((media) => {
                ...
                const mediaElement = $gallery[0].querySelector(
                  `[data-media-id="${media.id}"]`
                );
                if (mediaElement) {
                  const showMedia = !!(
                    currentColor && currentColor.includes(targetColor)
                  );
                  if (
                    mediaElement.parentElement.getAttribute("aria-hidden") ==
                    showMedia.toString()
                  ) {
                  ...
                  mediaElement.parentElement.setAttribute(
                    "aria-hidden",
                    !showMedia
                  );
                  ...
                  if (isCarouselLayout || theme.viewport.isXs()) {
                    if (showMedia) {
                      ...
                      const lazyImage =
                        mediaElement.querySelector(".lazyload--manual");
                      if (lazyImage) {
                  ...
                  if ($productThumbnails.length) {
                    const thumbnailElement =
                      $productThumbnails[0].querySelector(
    ...
    function updateSwiperSlidesPerView() {
      const visibleSlides = $gallery[0].querySelectorAll(
        '.theme-img:not([aria-hidden="true"])'
      );
      const swiperId = $gallery
        .find(".swiper-container:first")
        .attr("data-swiper-id");
      if (swiperId) {
        const thisSwiper = theme.swipers[swiperId];
        if (thisSwiper && thisSwiper.params) {
          thisSwiper.params.breakpoints[10000].slidesPerView =
            visibleSlides.length < 2 ? 1 : 2;
    ...
    $gallery.on("variantImageSelected", function (e, args) {
      ...
      if ($(this).find(".swiper-container-horizontal").length) {
        var swiperId = $(".swiper-container:first", this).attr(
          "data-swiper-id"
        );
        ...
        let isFirstSection =
          $container.closest(".shopify-section").index() === 0;
        ...
        if ($gallery.data("column-count") > 1 && underlineSelectedMedia) {
          $gallery
            ...
            .removeClass("product-media--active-variant");
          $gallery
            .find(`[data-media-id="${args.featured_media.id}"]`)
            .addClass("product-media--active-variant");
        }
      }
      ...
      setTimeout(() => {
        ...
        const $thumbSlider = $(
          `[data-section-id="${sectionId}"] .product-area__thumbs .carousel.slick-initialized`
        );
        if (
          $thumbSlider.length === 1 &&
          ($container.data("scroll-to-variant-media") !== false ||
            theme.viewport.isXs())
        ) {
          const $activeSlide = $thumbSlider.find(
            `[data-media-thumb-id="${args.featured_media.id}"]:first`
          );
    ...
    function initSwiper() {
      ...
      var swiperOpts = {
        ...
        spaceBetween:
          (isCarouselLayout && isGalleryNarrow) || isFeaturedProduct ? 20 : 0,
        ...
        on: {
          init: function () {
            ...
            if (theme.viewport.isXs()) {
              lazyLoadDelay =
                LocalStorageUtil.get("is_first_visit") === null ? 6000 : 2000;
            }
            ...
            if (
              theme.viewport.isXs() &&
              $(".product-detail__form__options a:not(.size-chart-link)")
                .length &&
              !isCarouselLayout
            ) {
              $(
          ...
          slideChangeTransitionEnd: function (e) {
            ...
            if (theme.viewport.isXs() || isCarouselLayout) {
              var $activeMedia = $gallery.find(
                ".swiper-slide-active .product-media"
              );
              var activeMediaObj =
                initialisedMedia[$activeMedia.data("media-id")];
      ...
      setTimeout(
        function () {
          ...
          if (theme.viewport.isSm() && !isCarouselLayout) {
            _this.showMedia(
              $swiperCont.find(
                ".swiper-slide.swiper-slide-active .product-media"
              ),
              false,
              true
            );
  ...
  theme.initContentSlider = function (target, afterChange) {
    $(".slideshow", target).each(function () {
      ...
      $(this)
        .on("init", function () {
          $(".slick-current .lazyload--manual", this)
            .removeClass("lazyload--manual")
            .addClass("lazyload");
        ...
        .slick({
          ...
          fade:
            $(this).data("transition") === "slide" && theme.viewport.isXs()
              ? false
              : true,
          speed: speed,
          ...
          customPaging: function (slider, i) {
            return (
              `<button class="custom-dot" type="button" data-role="none" role="button" tabindex="0">` +
          ...
          responsive: [
            {
              ...
              settings: {
                ...
                lazyLoad:
                  $(this).find("[data-lazy]").length > 0 ? "progressive" : null,
              },
            },
          ],
        })
        .on("beforeChange", function (event, slick, currentSlide, nextSlide) {
          ...
          const $unloadedImage = $(slick.$slides.get(nextSlide)).find(
            ".lazyload--manual"
          );
          ...
          if (
            $(this).data("transition") === "slide" ||
            $(this).data("transition") === "zoom"
          ) {
  ...
  theme.initProductSlider = function ($swiperCont) {
    ...
    let breakpoints = {
      ...
      900: {
        slidesPerView:
          slidesInView === 4 || slidesInView === 3 ? 2 : slidesInView,
      },
      1439: {
        slidesPerView:
          slidesInView === 4 || slidesInView === 3 ? 3 : slidesInView,
      },
      3000: {
    ...
    if ($clickies.length > 0) {
      ...
      for (
        ...
        for (var key in optionValues) {
          if (!optionValues[key]) {
            $(
              ".selector-wrapper:eq(" + optionIndex + ") .clickyboxes li a",
              container
            )
              .filter(function () {
      ...
      .each(function () {
        ...
        $pager.on("click", "a", function (e) {
          ...
          $.get($(this).attr("href"), function (data) {
            ...
            if (
              $(
                "[data-infinite-scroll-results].product-list--columns",
                container
              ).length
            ) {
              //Collage
              ...
              $newProducts.prepend(
                "<h2>" +
                  theme.strings.page.replace("{{ page }}", infiniteLoadCount) +
                  "</h2>"
              );

              var $newProducts = $newProducts.insertBefore(
                "[data-infinite-scroll-container] .pager-button"
              );
            } else {
              //Not collage
              var $newProducts = $data
                .find("[data-infinite-scroll-results] .product-block")
                .hide()
                .appendTo("[data-infinite-scroll-results]");
            }

            $("[data-infinite-scroll-container]", container)
            ...
            //Fix height
            if (!isCollage) {
              $("[data-infinite-scroll-results]", container).height(
                $("[data-infinite-scroll-results]", container).height()
              );
              ...
              var newHeight = 0;
              $("[data-infinite-scroll-results]", container)
                .children()
                .each(function () {
  ...
  theme.applyAjaxToProductForm = function ($formContainer) {
    ...
    $formContainer
      .filter('[data-ajax-add-to-cart="true"]:not(.feedback-go_to_cart)')
      ...
      .on("submit", function (e) {
        ...
        if (updateStickyButton) {
          ...
          $stickyBtn
            .data("originalHtml", $stickyBtn.html())
            .html(theme.strings.productAddingToCart);
        }
        ...
        $.post(
          ...
          function (itemData) {
            ...
            setTimeout(function () {
              $btn
                ...
                .html($btn.data("originalHtml"));
              if (updateStickyButton) {
                $stickyBtn
                  .removeAttr("disabled")
                  .removeClass("confirmation")
                  .html($stickyBtn.data("originalHtml"));
              }
            }, 4000);

            if (
              $form.hasClass("feedback-add_in_modal") ||
              $form.hasClass("feedback-add_in_modal_no_checkout")
            ) {
              const product = $.parseJSON(itemData);
              const noCheckoutButton = $form.hasClass(
              ...
              showThemeModal(
                [
                  ...
                  '<a href="' +
                    ...
                    `" class="button ${noCheckoutButton ? "" : "alt"}">` +
          ...
          if (
            ...
            if (response.errors && typeof response.errors === "object") {
              error = Object.entries(response.errors).map((item) =>
                item[1].join(", ")
              );
            }
            ...
            if (error) {
              ...
              errorArray.forEach((err, index) => {
                if (index > 0)
                  $statusMessageContainer[0].insertAdjacentHTML(
                    "beforeend",
                    "<br>"
                  );
  ...
  theme.OptionManager = new (function () {
    ...
    _.selectors = {
      ...
      priceAreaContainer: ".price-area-container",
      priceArea: ".price-area",
      kitArea: ".kit-area",
      variantIdInputs: '[name="id"]',
      submitButton:
        ".product-purchase-form [type=submit], .product-area__add-to-cart-xs button",
    ...
    _.getProductData = function ($form, productId) {
      ...
      if (!theme.productData[productId]) {
        theme.productData[productId] = JSON.parse(
          document.getElementById("cc-product-json-" + productId).innerHTML
        );
      }
      data = theme.productData[productId];
      if (!data) {
        console.log(
          "Product data missing (id: " + $form.data("product-id") + ")"
        );
      }
      return data;
    };
    ...
    _.updateInventoryNotice = function (variant, $container) {
      ...
      const $inventoryNoticeText = $container.find(
        ".product-inventory-notice__text"
      );
      const $inventoryNoticeIndicator = $container.find(
        ".product-inventory-notice__indicator"
      );

      if ($inventoryNotice.length) {
        const invCount = _._getVariantOptionElement(variant, $container).data(
          "inventory"
        );
        const invData = $inventoryNotice[0].dataset;

        const showInventoryCount =
          invData.showInventoryCount === "always" ||
          (invData.showInventoryCount === "low" &&
            invCount <= invData.inventoryThreshold);

        let notice;
        if (showInventoryCount) {
          notice = _._getString("inventoryNotice").replace(
            "[[ quantity ]]",
            invCount
          );
        } else {
        ...
        if ($inventoryNoticeIndicator.length === 1) {
          ...
          if (invCount >= invData.indicatorScale) {
            ...
            newWidth = (
              (100 / parseInt(invData.indicatorScale)) *
              invCount
            ).toFixed(1);
          }
          ...
          if (invCount <= parseInt(invData.inventoryThreshold)) {
            $bar
              .css("width", newWidth + "%")
              .css("background-color", invData.indicatorScaleColorBelow);
          } else {
            $bar
              .css("width", newWidth + "%")
              .css("background-color", invData.indicatorScaleColorAbove);
          }
        }
        ...
        if (
          invCount &&
          invCount > 0 &&
          (invData.showInventoryNotice === "always" ||
            invCount <= parseInt(invData.inventoryThreshold))
        ) {
          $inventoryNotice
    ...
    _.updatePrice = function (variant, $container) {
      ...
      if (variant) {
        // adding support for special prices on kit products
        var $priceAreaContainer = $container.find(
          _.selectors.priceAreaContainer
        );
        // checking for target container
        if ($priceAreaContainer.length) {
          var $specialPriceContainer =
            $priceAreaContainer.find(".special-price");

          // checking if kit has special price
          if (
            $specialPriceContainer.length &&
            $priceAreaContainer.attr("data-state") !== "done"
          ) {
            var $priceTooltip = $priceAreaContainer.find(".price-tooltip");
            var specialPrice =
              $specialPriceContainer.html().split("$")[1] * 100;
            var baseCurrency =
              $specialPriceContainer.attr("data-base-currency");
            var convertedSpecialPrice = theme.Shopify.convertCurrency(
              specialPrice,
              baseCurrency
            );
            var formattedMoney = theme.Shopify.formatMoney(
              convertedSpecialPrice,
              theme.money_format_with_code_preference
            );

            var saved = convertedSpecialPrice.toFixed(2) - variant.price;
            var formattedSaved = theme.Shopify.formatMoney(
              saved,
              theme.money_format_with_code_preference
            );
            // updating price area html
            $specialPriceContainer.html(formattedMoney);
            $priceTooltip.html(
              `<p>Congrats! You're saving <span>${formattedSaved}</span></p>`
            );
            $priceAreaContainer.attr("data-state", "done");
          }
        } else {
          var $kitContainer = $container.find(_.selectors.kitArea);
          if ($kitContainer.length) {
            var $kitSaveContainer = $kitContainer.find(
              ".kit-card__content-save"
            );
            if ($kitSaveContainer.length) {
              var kitPrice = $kitSaveContainer.attr("data-price") * 1;
              var kitSpecialPrice =
                $kitSaveContainer.attr("data-special-price").split("$")[1] *
                100;
              var baseCurrency = $kitSaveContainer.attr("data-base-currency");
              var kitTitle = $kitSaveContainer.attr("data-kit-title");

              var kitConvertedSpecialPrice = theme.Shopify.convertCurrency(
                kitSpecialPrice,
                baseCurrency
              );
              var saved = theme.Shopify.formatMoney(
                kitConvertedSpecialPrice.toFixed(2) - kitPrice,
                theme.money_format_with_code_preference
              );

              console.log("kit container", kitConvertedSpecialPrice);
              console.log(kitConvertedSpecialPrice, kitPrice);
              $kitSaveContainer.html(
                `Save ${saved} ⓘ
                <span class="kit-card__content-tooltip">${kitTitle} is <em>${theme.Shopify.formatMoney(kitConvertedSpecialPrice, theme.money_format_with_code_preference)}</em>. You're saving <em>${saved}</em> substantially.</span>`
              );
            }
          }
        }
        ...
        if (variant.unit_price_measurement) {
          ...
          $('<span class="unit-price__price theme-money">')
            .html(
              theme.Shopify.formatMoney(variant.unit_price, theme.money_format)
            )
        ...
        $priceArea.html(
          `<span class="current-price">${_.strings.priceNonExistent}</span>`
        );
      }
    };
    ...
    _.updateVariantOptionStatusClasses = function (variant, $container) {
      ...
      if (variant && variant.options && variant.options.length > 1) {
        ...
        $(".selector-wrapper", $container).each(function () {
          ...
          for (let [option, isAvailable] of Object.entries(
            optionStock[optionName]
          )) {
            option = removeDiacritics(option)
              .toLowerCase()
              .replace(/'/g, "")
              .replace(/[^a-z0-9]+/g, "-")
              .replace(/-+/g, "-")
    ...
    _.initProductOptions = function ($productForm) {
      ...
      $productForm
        .find(_.selectors.multiOption)
        .on("change.themeProductOptions", "select", function () {
          var selectedOptions = [];
          $(this)
            .closest(_.selectors.multiOption)
            .find("select")
            .each(function () {
      ...
      $productForm
        ...
        .on("change.themeProductOptions", function () {
          ...
          if ($radios.length) {
            // radio buttons
            $productForm
              .find(_.selectors.purchaseForm + " input")
              .filter(function () {
      ...
      $productForm.find(_.selectors.variantIdInputs).each(function () {
        ...
        $(this).on(
          ...
          function (e) {
            ...
            // string overrides
            var $addToCart = $container
              .find(_.selectors.submitButton)
            ...
            if (variant && variant.featured_media) {
              $container
                .find(_.selectors.gallery)
                .trigger("variantImageSelected", variant);
            }
            ...
            if (
              $productForm.find('[data-show-realtime-availability="true"]')
                .length > 0
            ) {
  ...
  jQuery(function ($) {
    $(document).on("click", "[data-cc-quick-buy]", function () {
      ...
      theme.currentQuickbuyRequest = $.get(ajaxUrl, function (response) {
        ...
        const $productDetail = $("<div>" + response + "</div>").find(
          ".section-product-template"
        );
        const $section = $productDetail.find(
        ...
        $productDetail
          .find(".product-detail__more_details a")
          .attr("href", productUrl);

        //Display the html
        $quickbuyModal
          .find(".theme-modal__loading")
          .replaceWith($productDetail);
  ...
  class ProductBlockInstance {
    ...
    showNextSlideImage() {
      this.hoverTimeout = setTimeout(() => {
        const slides = this.imageContainer.querySelectorAll(
          ".product-block--slide"
        );
    ...
    showSpecificSlideImage(imageUrl) {
      ...
      const nextSlide = this.imageContainer.querySelector(
        `.product-block--slide[src^="${imageUrlStart}"]`
      );
      if (nextSlide) {
        const currentSlide = this.imageContainer.querySelector(
    ...
    getImageUrl(url) {
      ...
      const imageContainerWidth = theme.device.isRetinaDisplay()
        ? this.productBlock.clientWidth * 2
    ...
    handleMouseEnterProductBlock(e) {
      ...
      if (!this.swatchImagesPreloaded) {
        this.productBlock
          .querySelectorAll(".cc-swatches a")
          .forEach((swatch) => {
    ...
    handleLeaveImageContainer(e) {
      ...
      if (this.imageSliderLoaded) {
        const activeSlide = this.imageContainer.querySelector(
          ".product-block--slide.-in"
        );
        if (activeSlide) {
    ...
    initImageSliderLoadingBar() {
      ...
      let transitionDuration =
        slides.length * this.slideDuration - loadingBarAnimateDelay;
    ...
    destroyImageSliderLoadingBar() {
      const loadingBar =
        this.productBlockImageContainer.querySelector(".loading-bar");
      if (loadingBar) {
        loadingBar.remove();
      }
    }
    ...
    handleWindowResize() {
      if (this.imageWidth && this.productBlock.clientWidth > this.imageWidth) {
        for (let i = 0; i < this.widths.length; i++) {
          if (
            this.widths[i] >= this.productBlock.clientWidth &&
            this.widths[i] > this.imageWidth
          ) {
  ...
  theme.HeaderSection = new (function () {
    ...
    handleScroll = function (nav, positioning, opacity) {
      if (
        opacity === "opaque_on_scroll" ||
        opacity === "opaque_on_scroll_alt"
      ) {
      ...
      if (
        (positioning === "peek" ||
          (nav.bar.isAnnouncementBar() && positioning == "sticky")) &&
        scrollTop > 100
      ) {
      ...
      if (
        (positioning == "sticky" || positioning == "peek") &&
    ...
    this.onSectionLoad = function (target) {
      ...
      $(
        ...
        target
      ).append('<span class="arr">' + theme.icons.chevronRight + "</span>");
  ...
  theme.FooterSection = new (function () {
    this.onSectionLoad = function (container) {
      ...
      container
        .querySelectorAll(".disclosure__toggle")
        .forEach((disclosureToggle) => {
    ...
    this.onSectionLoad = function (target) {
      ...
      if ($(".slideshow-blog", target).length) {
        theme.initContentSlider(target, function (slide) {
          $(".slideshow-blog__titles__active", target).removeClass(
            "slideshow-blog__titles__active"
          );
          $(`[data-go-to-slide="${slide}"]`, target)
            .parent()
        ...
        if ($('.slideshow[data-title-navigation="true"]', target).length) {
          function checkTitleNavHeight() {
            if (theme.viewport.isSm()) {
              $(".overlay-type .inner", target).css(
                "padding-bottom",
                $slideshowTitles.height() + 50 + "px"
              );
            } else {
  ...
  theme.ImageWithTextOverlay = new (function () {
    ...
    _.checkTextOverImageHeights = function () {
      $(
        ...
        var textVerticalPadding = parseInt(
          $(".overlay", this).css("padding-top")
        );
        var textHeight =
          $(".overlay__content", this).height() + textVerticalPadding * 2;
  ...
  theme.ProductTemplateSection = new (function () {
    ...
    this.onSectionLoad = function (target) {
      ...
      if (!isFeaturedProduct) {
          ...
          if (theme.viewport.isXs()) {
            if (!stickyAddToCartInitialised && !isQuickbuy) {
              $(window).on("throttled-scroll.sticky-add-to-cart", function () {
                if (
                  productSection.getBoundingClientRect().bottom <
                  $(window).outerHeight()
                ) {
              ...
              $(".product-area__add-to-cart-xs button", target).on(
                "click",
                function (e) {
                  const btnSubmit = document.querySelector(
                    '.product-detail__form button[type="submit"]'
                  );
        ...
        $(window).on(
          `debouncedresizewidth.productDetails${sectionUniqueId}`,
          resizeProductDetails
        );
        $(window).on(
          `cc-header-updated.productDetails${sectionUniqueId}`,
          resizeProductDetails
        );
        $(window).on(
          `shopify:section:reorder.productDetails${sectionUniqueId}`,
          resizeProductDetails
        );
        resizeProductDetails();
      }
      ...
      $(window).on(
        `cc-variant-updated.product-swatches${sectionUniqueId}`,
        (e, args) => {
          ...
          if ($swatchesContainer.length) {
            ...
            $swatchesContainer
              .find("label")
              .append(
                `<span class="cc-swatches__label">${$swatchesContainer.find(".active").text()}</span>`
              );
          }
        }
      );
    ...
    this.functions = {
      ...
      updateSearchQuery: function (e) {
        ...
        if ($form.length) {
          ...
          $form
            .find('[name="q"]')
            .val($(e.currentTarget).find('[name="q"]').val());
        ...
        evt.currentTarget.querySelectorAll("input, select").forEach((input) => {
          if (
            ...
            if (input.value === "" && input.dataset.currentValue) {
              queryVals.push([
                input.name,
                encodeURIComponent(input.dataset.currentValue),
              ]);
            } else {
      ...
      refreshSelects: function () {
        $("select:not(.original-selector)", this.$container)
          .selectReplace()
          .closest(".selector-wrapper")
          .addClass("has-pretty-select");
      },
      ...
      ajaxLoadUrl: function (url, noPushState) {
        ...
        if (!noPushState) {
          ...
          if (fullUrl.slice(0, 1) === "/") {
            fullUrl =
              window.location.protocol + "//" + window.location.host + fullUrl;
          }
        ...
        $ajaxContainers
          .find(".product-list")
          .append(
            `<span class="loading" aria-label="${theme.strings.loading}">${theme.icons.loading} </span>`
          );
        ...
        this.currentAjaxLoadUrlFetch = $.get(
          ...
          function (data) {
            ...
            // replace contents
            const $newAjaxContainers = $(`<div>${data}</div>`).find(
            ...
            $newAjaxContainers.each(function (index) {
              ...
              $($ajaxContainers[index])
                .find(".cc-accordion-item")
                .each(function () {
                  const accordionIndex = $(this)
                    .closest(".cc-accordion")
                    .index();
                  if ($(this).hasClass("is-open")) {
                    $newAjaxContainer
                      .find(
                        `.cc-accordion:nth-child(${accordionIndex + 1}) .cc-accordion-item`
                      )
                      .addClass("is-open")
                      .attr("open", "");
                  } else {
              ...
              if (
                $(".cc-product-filter", _this.$container).length &&
                $(".cc-product-filter", _this.$container).hasClass("-in")
              ) {
            ...
            if ($filterSidebar.length && $filterSidebar.hasClass("-in")) {
              let buttonText;
              let resultCount = $(".product-list", _this.$container).data(
                "result-count"
              );

              if (resultCount === 1) {
                buttonText = $filterBtn
                  .data("result-count-text-singular")
                  .replace("[x]", resultCount);
              } else {
                buttonText = $filterBtn
      ...
      if ($carouselGallery.length) {
        var assessCarouselFunction = function () {
          ...
          if (!shouldShowCarousel) {
            $(".lazyload--manual", $carouselGallery)
              .removeClass("lazyload--manual")
              .addClass("lazyload");
          }
        ...
        $(window).on(
          "debouncedresize.themeSection" + container.id,
          assessCarouselFunction
        );
      }
    };
  ...
  theme.AccordionSection = new (function () {
    ...
    this.onBlockSelect = function (container) {
      const accordionElem = container.querySelector(
  ...
  jQuery(function ($) {
    ...
    $(window).on("debouncedresize slideshowfillheight", function (e) {
      ...
      $(".slideshow.fill-viewport, .slideshow.fill-viewport .slide").css(
        "min-height",
        desiredHeight
      );
      ...
      if (
        $("body.header-section-overlap").length &&
        nav.bar.isAnnouncementBar()
      ) {
    ...
    $(window).on("scroll assessFeatureHeaders", function () {
      ...
      $(".feature-header:not(.feature-header--visible)")
        .filter(function () {
    ...
    updateNavCtas = function () {
      ...
      let $ctasToShow = $(
        `#page-menu .nav-ctas__container[data-for-nav-item="${trimmedTitle}"]:hidden`
      );

      if ($ctasToShow.length > 0) {
        if ($("#page-menu .nav-ctas__container:visible").length) {
          $("#page-menu .nav-ctas__container:visible").fadeOut(
            drilldownTransSpeed,
            function () {
    ...
    $(document).on(
      ...
      function () {
        ...
        //- Links
        var $content = $('<div class="container growth-area"/>').append(
          $(this)
            .siblings("ul")
            .clone()
        ...
        if (theme.strings.back.length > 0) {
          $menuList.prepend(
            `<li class="main-nav__back" data-nav-title="${trimmedTitle}"><a href="#" data-revert-back><span class="arr arr--left">${theme.icons.chevronLeft}</span>${theme.strings.back}</a></li>`
          );
        }
        ...
        $containers
          .addClass("inactive")
          .fadeOut(drilldownTransSpeed, function () {
            $content
              .hide()
              .insertAfter($containers.last())
              .fadeIn(drilldownTransSpeed);
        ...
        $("#page-menu > .inner > .nav-container > .nav-footer-links").fadeOut(
          drilldownTransSpeed
        );
    ...
    $(document).on(
      ...
      function () {
        ...
        ).fadeOutAndRemove(drilldownTransSpeed, function () {
          var $menuToShow = $(
    ...
    $(document).on(
      ...
      function (e) {
        ...
        if (
          e.type == "click" &&
          thisInteractionTimestamp - theme.lastHoverInteractionTimestamp < 500
        ) {
          return false;
        }
        ...
        if (e.type != "forceopen") {
          $tierCont.children().each(function () {
            if (
              parseInt($(this).attr("class").split("-")[1]) >= targetTierNum
            ) {
    ...
    theme.closeOpenMenuItem = function () {
      ...
      $(".multi-level-nav.reveal-on-hover .has-children.expanded").trigger(
        "forceclose"
      );
    };
    ...
    window.closeThemeModal = function (immediate, callbackFn) {
      ...
      if (
        $("html.supports-transforms").length &&
        (typeof immediate == "undefined" || !immediate)
      ) {
    ...
    window.showInPageModal = function ($target) {
      ...
      $("body")
        ...
        .attr("data-modal-id", $target.attr("id"));
    ...
    $("#template textarea").each(function () {
      $(this).autogrow({ animate: false, onInitialize: true });
    });
    ...
    if (
      theme.getUrlParameter("customer_posted") ||
      (formType && formType === "customer")
    ) {
      if (
        window.location.hash &&
        window.location.hash === "footer_newsletter_signup"
      ) {
        setTimeout(() => {
          $("html,body").animate(
            {
    ...
    $(document).on("click", "[data-cc-animate-click]", function (e) {
      if (
        (theme.settings.animationEnabledDesktop && theme.viewport.isSm()) ||
        ...
        if (
          ...
          if (isLoadingAnimation) {
            setTimeout(() => {
              $veil
                .addClass("cc-veil--animate--in")
                .addClass("cc-veil--animate--running");
    ...
    const firstSection = document.body.querySelector(
